...
![Croptop instructions for people to fork this fundraising solution](croptop_fork_1.png)
...
2. 点击 "Make a new Juicebox"，可以快捷创建一个 Juicebox 项目，以便大家发布内容时向你支付费用。快捷创建流程中需要填写 NFT 系列的名称及符号，还有这个 Juicebox 项目的项目方地址。另外，还可以设定在你的项目上发布内容的最低价格、最低发行量和最大发行量等限制条件。人们在你的项目上发布内容时，可以自行设定该内容的价格及发行量等参数，但这些参数都必须符合你作为项目方最早设定的限制条件。
   ...
   ![Croptop shortcut to create a Juicebox project](croptop_fork_shortcut.png)

3. 下一步就是授予 Croptop 合约在你的项目上相当于操作员的发布权限。上一步创建项目完成之后，获得项目的 ID 并在这里填写并确定，就可以授予 Croptop 在项目上发布 NFT 的权限。

   ![Croptop to give permission to post](croptop_fork_permission.png)

4. 最后一步是可选项，设置可以使用 Croptop 在你的 Juicebox 项目上发布内容的钱包地址，以及可以发布 NFT 的类别。

   ![Croptop to set allowed list for posting](croptop_fork_allowed.png)
...
![Delegation page on project settings home](Peel_delegation_page.png)
...
![Project NFT Theme setting page](project_NFT_theme.png)
...
![Juice Cards beta site](juice_cards_beta.png)
...
![Proposal to fund more open rounds on prop house](proposal_396.png)
...
![JBX sponsorship in water polo T-shirts](mcsa_camiseta.png)