...
_The msg.sender must be an [`IJBSingleTokenPaymentTerminal`](/docs/dev/v2/interfaces/ijbpaymentterminal.md). The amount specified in the params is in terms of the msg.sender's tokens._
...
* Arguments:
  ...
  * `_amount` is a [`JBTokenAmount`](/docs/dev/v2/data-structures/jbtokenamount.md) data structure specifying the amount of tokens being paid. Includes the token being paid, the value, the number of decimals included, and the currency of the amount.
...
4.  Create a variable where the weight to use in subsquent calculations will be saved.
    ...
    uint256 _weight;
    ```
5.  If the project's current funding cycle is configured to use a data source when receiving payments, ask the data source for the parameters that should be used throughout the rest of the function given provided contextual values in a [`JBPayParamsData`](/docs/dev/v2/data-structures/jbpayparamsdata.md) structure. Otherwise default parameters are used.
    ...
    * [`payParams`](/docs/dev/v2/interfaces/ijbfundingcycledatasource.md)
