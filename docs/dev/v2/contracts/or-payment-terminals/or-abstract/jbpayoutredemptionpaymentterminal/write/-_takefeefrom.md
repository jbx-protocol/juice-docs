...
* Arguments:
  ...
  * `_fundingCycle` is the [`JBFundingCycle`](/docs/dev/v2/data-structures/jbfundingcycle.md) during which the fee is being taken.
...
2.  If the funding cycle is configured to hold fees, add a [`JBFee`](/docs/dev/v2/data-structures/jbfee.md) data structure to the project's stored held fees to be either processed or refunded later, and emit a `HoldFee` event with the relevant parameters. Otherwise, take the fee and emit a `ProcessFee` event with the relevant parameters.
    ...
    * [`HoldFee`](/docs/dev/v2/contracts/or-payment-terminals/or-abstract/jbpayoutredemptionpaymentterminal/events/holdfee.md)
