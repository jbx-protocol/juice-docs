...
* Arguments:
  ...
  * `_to` is the [`IJBMigratable`](/docs/dev/v2/interfaces/ijbmigratable.md) controller to which the project is migrating.
* Through the [`requirePermission`](/docs/dev/v2/contracts/or-abstract/jboperatable/modifiers/requirepermission.md) modifier, the function is only accessible by the project's owner, or from an operator that has been given the [`JBOperations.MIGRATE_CONTROLLER`](/docs/dev/v2/libraries/jboperations.md) permission by the project owner for the provided `_projectId`.
...
4.  Distribute any outstanding reserved tokens. There are reserved tokens to be distributed if the tracker does not equal the token's total supply.
    ...
    * [`_processedTokenTrackerOf`](/docs/dev/v2/contracts/or-controllers/jbcontroller/properties/-_processedtokentrackerof.md)
    * [`_distributeReservedTokensOf`](/docs/dev/v2/contracts/or-controllers/jbcontroller/write/-_distributereservedtokensof.md)

    _External references:_

    * [`totalSupplyOf`](/docs/dev/v2/contracts/jbtokenstore/read/totalsupplyof.md)
5.  Let the new controller know that a migration to it is happening.
    ...
    * [`prepForMigrationOf`](/docs/dev/v2/contracts/or-controllers/jbcontroller/write/prepformigrationof.md)
...
7.  Emit a `Migrate` event with the relevant parameters.
    ...
    * [`Migrate`](/docs/dev/v2/contracts/or-controllers/jbcontroller/events/migrate.md)
