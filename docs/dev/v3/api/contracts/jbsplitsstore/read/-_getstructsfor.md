...
* The function returns an array of [`JBSplit`](/docs/dev/v3/api/data-structures/jbsplit.md)s.
...
2.  Inititalize an array of [`JBSplit`](/docs/dev/v3/api/data-structures/jbsplit.md) with length equal to the number of splits expected.
...
3.  For each index, parse out the packed split parts into [`JBSplit`](/docs/dev/v3/api/data-structures/jbsplit.md) structs and add to the array. The packed splits are stored in two different `uint256` slots, the second of which contains info that is populated way less frequently.
