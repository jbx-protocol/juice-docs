...
For projects deploying omnichain, the [`JBOmnichainDeployer.launchProjectFor(...)`](/docs/dev/v5/api/omnichain-deployers/JBOmnichainDeployer.md#launchprojectfor) transaction is a wrapper that also takes in information about deploying [suckers](/docs/dev/v5/learn/glossary/omnichain.md) for each chain pair.
...
*   For `rulesetConfigurations` send the following [`JBRulesetConfig`](/docs/dev/v5/api/core/structs/JBRulesetConfig.md).
