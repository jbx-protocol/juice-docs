...
But note that this `_amount` used when calling `_refundHeldFees` is the same `_amount` being added to the balance. In other words, the held fee refund calculation is being made based on the amount being added back to the project – not the amount which was taken out. Even if a project creator returns all the funds they have received to a project, that will only amount to 1 - fee% (currently 97.5%) of the amount "paid out" (since they never received the "fee amount", but the fee amount was included in the `JBFee` struct's `amount`).
