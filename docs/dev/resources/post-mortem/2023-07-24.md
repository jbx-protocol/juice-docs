...
Note that the `_amount` here is based on the full amount being paid out from the project, including the fee.
...
But the `_amount` used when calling `_refundHeldFees` is only the amount being added to the balance. In other words, the held fee refund calculation is being made based on the ETH amount being returned to the project – not the amount which was taken out. Even if a project creator returns all the funds they have received to a project, that will only amount to 100% minus the protocol fee (currently 100% - 2.5%, or 97.5%) of the amount paid out (since they never received the "fee amount".
