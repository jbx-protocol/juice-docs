...
1. 🧠🧠🧠.eth deployed the [Legends](https://juicebox.money/v2/p/548) project to raise funds for an auction. The project was deployed with an unlimited payout to the project owner and held fees.
2. People paid the project 12.35 ETH.
...
4. People paid the project 1 more ETH.
5. 10.097560975609756 ETH was [transferred back to the project](https://etherscan.io/tx/0xa8c50fb7db1f187949de35fbe81ffe48139803fa3885ef4a62a478f5e3d83a41) (97 wei less).
6. The project only had 11.343842950624628098 ETH of overflow (instead of 11.35 ETH minus 97 wei).

Because of this, the project's supporters received slightly less ETH than a full refund should entail.

The remaining 0.006157049375371805 ETH are still held as fees (as demonstrated by [this simulation](https://dashboard.tenderly.co/shared/simulation/fcf9cb9f-f682-4998-a68a-cb6de53cdaf6)). The reason they were held is because of an error in [`JBPayoutRedemptionPaymentTerminal3_1`](https://github.com/jbx-protocol/juice-contracts-v3/blob/48fe7091a30761fa42ce394c68aad2fcf639ea53/contracts/JBETHPaymentTerminal3_1.sol)'s hold fees logic.
...
These funds can be returned later with `_refundHeldFees`:
...
for (uint256 _i; _i < _heldFeesLength; ) {
  if (leftoverAmount == 0) _heldFeesOf[_projectId].push(_heldFees[_i]);
    ...
    unchecked {
      leftoverAmount = leftoverAmount - _heldFees[_i].amount; // This line subtracts the full amount
```

`_refundHeldFees` is invoked when `_addToBalanceOf` is called with `_shouldRefundHeldFees` set to true:

```
uint256 _refundedFees = _shouldRefundHeldFees ? _refundHeldFees(_projectId, _amount) : 0;
```

But note that this `_amount` used when calling `_refundHeldFees` is the same `_amount` being added to the balance. In other words, the held fee refund calculation is being made based on the amount being added back to the project – not the amount which was taken out. Even if a project creator returns all the funds they have received to a project, that will only amount to 100% minus the protocol fee (currently 100% - 2.5%, or 97.5%) of the amount "paid out" (since they never received the "fee amount", but the fee amount was included in the `JBFee` struct's `amount`).
...
I (filipv) refunded individuals according to their contributions in transaction [`0x54cff0ab259f8f10fa562bf6fff0999ed668a5d6b96a23994610a0a33333406e`](https://etherscan.io/tx/0x54cff0ab259f8f10fa562bf6fff0999ed668a5d6b96a23994610a0a33333406e):
