...
* The function returns an array of [`JBSplit`](/dev/deprecated/v2/data-structures/jbsplit.md)s.
...
1.  Get a reference to the expected number of splits for the specified domain and group.
    ...
    * [`_splitCountOf`](/dev/deprecated/v2/contracts/jbsplitsstore/properties/-_splitcountof.md)
2.  Inititalize an array of [`JBSplit`](/dev/deprecated/v2/data-structures/jbsplit.md) with length equal to the number of splits expected.

    ```
    // Initialize an array to be returned that has the set length.
    JBSplit[] memory _splits = new JBSplit[](_splitCount);
    ```
3.  For each index, parse out the packed split parts into [`JBSplit`](/dev/deprecated/v2/data-structures/jbsplit.md) structs and add to the array. The packed splits are stored in two different `uint256` slots, the second of which contains info that is populated way less frequently.
    ...
    * [`_packedSplitParts1Of`](/dev/deprecated/v2/contracts/jbsplitsstore/properties/-_packedsplitparts1of.md)
    * [`_packedSplitParts2Of`](/dev/deprecated/v2/contracts/jbsplitsstore/properties/-_packedsplitparts2of.md)
