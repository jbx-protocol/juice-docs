...
* The function returns an array of [`JBSplit`](/docs/v4/deprecated/v2/data-structures/jbsplit.md)s.
...
2.  Inititalize an array of [`JBSplit`](/docs/v4/deprecated/v2/data-structures/jbsplit.md) with length equal to the number of splits expected.

    ```
    // Initialize an array to be returned that has the set length.
    JBSplit[] memory _splits = new JBSplit[](_splitCount);
    ```
3.  For each index, parse out the packed split parts into [`JBSplit`](/docs/v4/deprecated/v2/data-structures/jbsplit.md) structs and add to the array. The packed splits are stored in two different `uint256` slots, the second of which contains info that is populated way less frequently.
