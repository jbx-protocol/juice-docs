...
:::note
To reduce gas fees, newly issued $JBX tokens are stored in the Juicebox [TicketBooth](/docs/v4/deprecated/v1/ticketbooth/README.md) contract by default ("staked"). $JBX holders can call the `unstake` function on the TicketBooth contract to mint $JBX ERC-20 tokens to their wallets. In the frontend, this is called `Claim` and can be found under the `Manage` button.
