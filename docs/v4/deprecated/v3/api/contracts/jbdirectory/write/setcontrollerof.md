...
* Through the [`requirePermissionAllowingOverride`](/v4/deprecated/v3/api/contracts/or-abstract/jboperatable/modifiers/requirepermissionallowingoverride.md) modifier, the function is only accessible by the project's owner, from an operator that has been given the [`JBOperations.SET_CONTROLLER`](/v4/deprecated/v3/api/libraries/jboperations.md) permission by the project owner for the provided `_projectId`, from the project's current controller, or from an allow-listed controller if the project doesn't already have a controller set.
...
1.  Project IDs are assigned incrementally. If the provided project ID is greater than the number of projects, it must not be a valid project ID.
    ...
    * [`projects`](/v4/deprecated/v3/api/contracts/jbdirectory/properties/projects.md)
...
2.  Get a reference to the project's current funding cycle.
    ...
    * [`fundingCycleStore`](/v4/deprecated/v3/api/contracts/jbdirectory/properties/fundingcyclestore.md)
...
5.  Emit a `SetController` event with the relevant parameters.
    ...
    * [`SetController`](/v4/deprecated/v3/api/contracts/jbdirectory/events/setcontroller.md)
