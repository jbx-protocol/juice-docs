...
_The msg.sender must be an [`IJBSingleTokenPaymentTerminal`](/v4/deprecated/v3/api/interfaces/ijbpaymentterminal.md). The amount specified in the params is in terms of the msg.sender's tokens._
...
* Arguments:
  ...
  * `_amount` is a [`JBTokenAmount`](/v4/deprecated/v3/api/data-structures/jbtokenamount.md) data structure specifying the amount of tokens being paid. Includes the token being paid, the value, the number of decimals included, and the currency of the amount.
...
5.  If the project's current funding cycle is configured to use a data source when receiving payments, ask the data source for the parameters that should be used throughout the rest of the function given provided contextual values in a [`JBPayParamsData`](/v4/deprecated/v3/api/data-structures/jbpayparamsdata.md) structure. Otherwise default parameters are used.
    ...
    * [`payParams`](/v4/deprecated/v3/api/interfaces/ijbfundingcycledatasource.md)
